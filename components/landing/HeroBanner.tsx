...
export default function HeroBanner({ homeData }: HeroBannerProps) {
  ...
  const videoId = (() => {
    ...
    const match = videoUrl.match(/(?:v=|youtu\.be\/|embed\/|shorts\/)([^&?/]+)/);
            ...
            <div className={`order-2 text-center ${textAlign}`}>
              ...
              <h1 className="mx-auto max-w-4xl text-[1.75rem] font-black leading-[1.34] tracking-[-0.015em] text-white sm:text-[2.25rem] lg:mx-0 lg:text-[2.55rem] xl:text-[2.95rem]">
                {apiHero ? (
                  ...
                  <>
                    {hero.titleLine1}
                    <span className="mt-2 block text-[var(--mint)]">{hero.titleHighlight}</span>
                    {hero.titleLine2}
                  </>
                )}
              </h1>
              ...
              <div className={`mt-6 flex flex-wrap justify-center gap-3 sm:mt-8 ${flexAlign}`}>
                <button
                  type="button"
                  onClick={() =>
                    runLandingButtonAction(ctaPrimaryUrl, {
                      onFallback: () => setBookingOpen(true),
                ...
                <button
                  type="button"
                  onClick={() =>
                    runLandingButtonAction(ctaSecondaryUrl, {
                      onFallback: () => document.getElementById('works')?.scrollIntoView({ behavior: 'smooth' }),
                      navigate: router.push,
                    })
                  }
              ...
              <div className={`mt-6 flex flex-wrap justify-center gap-2 sm:mt-8 ${flexAlign}`}>
                {proofTags.map((label) => (
                  <span
                    key={label}
                    className="rounded-full border border-white/10 bg-white/7 px-4 py-2 text-xs font-bold text-slate-200 backdrop-blur"
                  >
                    {label}
                  </span>
                ))}
              ...
              <div className="relative mx-auto w-full max-w-[420px] sm:max-w-[540px] lg:max-w-[620px]">
                <div className="overflow-hidden rounded-2xl border border-white/15 shadow-lg shadow-black/25 sm:rounded-3xl">
                  <div className="relative aspect-[16/10] w-full min-h-[220px] sm:min-h-[300px] lg:min-h-[360px]">
                    {videoLoaded ? (
                      <iframe
                        src={`https://www.youtube.com/embed/${videoId}?start=${HERO_VIDEO_START}&autoplay=1&playsinline=1&rel=0&modestbranding=1`}
                      ...
                      >
                        <img
                          ...
                          // maxres isn't generated for every video; YouTube then serves a 120px placeholder or errors.
                          onLoad={(e) => {
                        ...
                        />
                        <span className="relative grid h-16 w-16 place-items-center rounded-full bg-primary text-2xl shadow-2xl shadow-cyan-950/50 transition-transform duration-300 group-hover:scale-110">

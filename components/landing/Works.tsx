            ...
            ? apiData!.capabilities.map((cap, i) => (
                ...
                >
                  ...
                  <div className="p-5 sm:p-6">
                    ...
                    <p className="text-sm leading-relaxed text-slate-600 dark:text-slate-300 sm:min-h-12">{cap.description}</p>
                    {cap.tags.length > 0 ? (
                      <div className="mt-5 flex flex-wrap gap-2">
                        {cap.tags.map((tag) => (
                          <span key={tag.id} className="rounded-full bg-primary/10 px-3 py-1.5 text-xs font-bold text-primary">
                            {tag.name}
                          </span>
                        ))}
                      </div>
